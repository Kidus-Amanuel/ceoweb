...
export async function POST(req: Request) {
  try {
    ...
    if (mappingError || !tenantMapping) {
      console.log(
        `[Fleet] Creating new Traccar User for Company: ${profile.company_id}`,
      );
      try {
        ...
        return NextResponse.json(
          { error: "Failed to provision fleet tenant" },
          { status: 500 },
        );
      }
    } else {
    ...
    console.log(
      `[Fleet] Requesting Session Token for Traccar User: ${traccarUserId}`,
    );
