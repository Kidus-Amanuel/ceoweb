...
export async function POST(req: Request) {
  try {
    ...
    const positions = Array.isArray(payload.positions) ? payload.positions : [payload.position].filter(Boolean);
    ...
    if (positions.length === 0) {
      return NextResponse.json({ status: 'ignored', message: 'No position data found' });
    }
    ...
    for (const pos of positions) {
      ...
      if (mappingError || !mapping) {
        console.warn(`[Traccar Webhook] No mapping found for Traccar Device ID: ${deviceId}`);
        continue;
      }
      ...
      if (updateError) {
        console.error(`[Traccar Webhook] Failed to update vehicle ${mapping.erp_vehicle_id}:`, updateError);
      } else {
        console.log(`[Traccar Webhook] Updated vehicle ${mapping.erp_vehicle_id} location`);
      }
    }
