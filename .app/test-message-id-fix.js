...
(async () => {
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    const hasDuplicateKeys = logs.some(
      (log) => log.includes("Duplicate key") || log.includes("same key"),
    );

    if (hasDuplicateKeys) {
      console.error(
        `Error: Duplicate keys found after sending question ${i + 1}: "${question}"`,
      );
    } else {
      console.log(
        `Success: No duplicate keys after sending question ${i + 1}: "${question}"`,
      );
    }
    ...
    if (uniqueIds.length !== messages.length) {
      console.error(
        `Error: Duplicate message ids found: ${messages.join(", ")}`,
      );
    } else {
      console.log(
        `Success: All messages have unique ids: ${messages.join(", ")}`,
      );
    }
  }
