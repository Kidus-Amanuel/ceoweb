...
const moduleConfig: Record<string, { table: string; fields: string[] }> = {
  ...
  hr: { 
    table: 'employees', 
    fields: ['id', 'name', 'email', 'phone', 'department', 'status', 'created_at'] 
  },
  finance: { 
    table: 'invoices', 
...
export async function readModuleData(module: string, filters: any = {}, traceId?: string) {
  ...
  try {
    ...
    if (!allowedModules.includes(module)) {
      console.error(`User type ${userType} does not have permission to access module:`, module);
    ...
    console.log(`Query returned ${Array.isArray(data) ? data.length : 0} records`);
