  ...
  crm: {
    ...
    entityConfig: {
      customers: {
        ...
        relationships: {
          deals: {
            ...
            fields: ["id", "title", "stage", "value", "status"],
            displayField: "title",
          },
          activities: {
            table: "activities",
      ...
      deals: {
        ...
        displayColumns: ["Id", "Title", "Stage", "Value", "Customer"],
        relationships: {
          customer: {
            table: "customers",
            foreignKey: "customer_id",
...
async function checkCustomFieldsColumn(
  supabase: any,
  table: string,
  ...
  try {
    ...
    console.error(
      `Error checking custom fields column '${customFieldsColumn}' in table '${table}':`,
      error,
    );
    return false;
  }
...
export async function readModuleData(
  ...
  try {
    ...
    const table = config.entityTypes?.includes(entityType)
      ? entityType
      : config.defaultTable;
    ...
    if (config.metadataKey) {
      ...
      if (!companyError && company) {
        try {
          ...
          customColumns = (metadata[entityType] ?? []).filter(
          ...
          console.log(
            "Custom columns found:",
            customColumns.map((c) => c.field_label),
          );
        } catch (error) {
    ...
    const hasCustomFieldsColumn = config.customFieldsColumn
      ? await checkCustomFieldsColumn(
          supabase,
          table,
          config.customFieldsColumn,
        )
      : false;
    if (
      !hasCustomFieldsColumn &&
      config.customFieldsColumn &&
      fields.includes(config.customFieldsColumn)
    ) {
      fields = fields.filter((field) => field !== config.customFieldsColumn);
      console.log(
        `Custom fields column '${config.customFieldsColumn}' not found, removed from query`,
      );
    }
    ...
    data.forEach((item: any, index: number) => {
      if (item[customFieldsColumn]) {
        console.log(
          `Record ${index + 1} ${customFieldsColumn}:`,
          item[customFieldsColumn],
        );
      } else {
    ...
    if (
      includeRelated &&
      config.entityConfig &&
      config.entityConfig[entityType]?.relationships
    ) {
      ...
      processedData = await Promise.all(
        resultsToReturn.map(async (item: any) => {
          ...
          for (const [relationName, relationConfig] of Object.entries(
            ...
            try {
              console.log(
                `Fetching ${relationName} data for item ${item.id}...`,
              );

              // Query related table
              // For relationships, the foreign key is in the current item, not the related table
              const { data: relatedData, error: relatedError } = await supabase
                .from(relationConfig.table)
                .select(relationConfig.fields.join(","))
                .eq("id", item[relationConfig.foreignKey]) // Fix: Use item's foreign key value instead of item.id
                .is("deleted_at", null);

              if (relatedError) {
                console.error(
                  `Error fetching ${relationName} data:`,
                  relatedError,
                );
                continue;
              }
    ...
    const cleanedData = processedData.map((item: any) => {
      ...
      // Replace customer_id with customer name for deals
      if (entityType === "deals" && newItem.customer) {
        newItem["Customer"] = newItem.customer.name;
        delete newItem.customer_id;
      }
      ...
      if (config.customFieldsColumn && newItem[config.customFieldsColumn]) {
        try {
          const customFields =
            typeof newItem[config.customFieldsColumn] === "string"
          ...
          if (typeof customFields === "object" && customFields !== null) {
            Object.entries(customFields).forEach(([key, value]) => {
              ...
              const columnDef = customColumns.find(
                (col) => col.field_name === key,
              );
    ...
    if (cleanedData.length > 0) {
      insights.activeCount = (cleanedData as any[]).filter(
        (item) => item.status === "active",
      ).length;
      insights.recentCount = (cleanedData as any[]).filter(
        (item) => new Date(item.created_at) >= thirtyDaysAgo,
      ).length;

      const totalNameLength = (cleanedData as any[]).reduce(
        (sum: number, item: any) => sum + (item.name || "").length,
        0,
      );
      insights.averageNameLength = Math.round(
        totalNameLength / cleanedData.length,
      );
    }
