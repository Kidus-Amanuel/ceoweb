...
async function testGoogleAPI() {
  try {
    ...
    if (!initialResponse.ok) {
      console.error(
        `Initial API Error: ${initialResponse.status} ${initialResponse.statusText}`,
      );
    ...
    if (initialData.candidates && initialData.candidates.length > 0) {
      ...
      if (
        ...
        if (functionCall.name === "read_module_data") {
          functionResult = await readModuleData(
        ...
        if (!followupResponse.ok) {
          console.error(
            `Follow-up API Error: ${followupResponse.status} ${followupResponse.statusText}`,
          );
        ...
        if (followupData.candidates && followupData.candidates.length > 0) {
          ...
          if (
            ...
            finalCandidate.content.parts.length > 0
          ) {
            console.log(
