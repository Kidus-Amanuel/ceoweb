...
export const AIMarkupRenderer: React.FC<{
  ...
  const tableRegex = /<table\s+columns=(["'])([^"']+)\1\s+rows=(["'])([\s\S]*?)\3\s*\/>/g;
  ...
  const matches: { type: "table" | "button"; index: number; match: RegExpExecArray }[] = [];
    ...
    if (match.index > lastIndex) {
     ...
     if (type === "table") {
      try {
        ...
        let rows;
         ...
         if (match[4].trim() === "invalid_json") {
           console.log("Received invalid JSON placeholder, rendering error message");
           parts.push(
             <div key={`error-${match.index}`} className="text-red-500 bg-red-50 p-2 rounded">
         ...
         console.log("JSON parsing failed, attempting to replace single quotes");
         ...
         normalizedRows = normalizedRows
           .replace(/([^\[\],"']+)/g, (matchStr) => {
             ...
             if (/^\d+(\.\d+)?$/.test(trimmed) || trimmed.toLowerCase() === "true" || trimmed.toLowerCase() === "false") {
               return matchStr;
             }
             // Quote it
             return `"${trimmed}"`;
         ...
         try {
           ...
           console.error("Failed to parse rows even with quoted strings:", fallbackError);
           parts.push(
             <div key={`error-${match.index}`} className="text-red-500 bg-red-50 p-2 rounded">
      ...
      const allowedColumns = {
        ...
        hr: ["Id", "Name", "Email", "Phone", "Department", "Status", "Created At"],
      ...
      if (columns.some(col => col.toLowerCase().includes("category"))) moduleType = "inventory";
      if (columns.some(col => ["type", "model"].includes(col.toLowerCase()))) moduleType = "fleet";
      if (columns.some(col => col.toLowerCase().includes("department"))) moduleType = "hr";
      if (columns.some(col => ["number", "amount"].includes(col.toLowerCase()))) moduleType = "finance";
      
      // Filter out invalid columns (case-insensitive)
      const validColumns = columns.filter(col => 
        allowedColumns[moduleType].some(allowedCol => allowedCol.toLowerCase() === col.toLowerCase())
      );
      const validRows = rows.map((row: any[]) => {
        return row.filter((_: any, idx: number) => validColumns.includes(columns[idx]));
      });
      ...
      if (validColumns.length < columns.length) {
        console.warn(`Removed ${columns.length - validColumns.length} invalid columns from table`);
      }
      ...
      parts.push(
        <div key={`table-${match.index}`} className="mb-4">
          <Table>
            <TableHeader>
              <TableRow>
                {validColumns.filter(col => col !== "Id").map((col, idx) => (
            ...
            <TableBody>
              {validRows.map((row: any[], rowIdx: number) => (
                <TableRow key={rowIdx}>
                  {row.filter((cell: any, cellIdx: number) => validColumns[cellIdx] !== "Id").map((cell: any, cellIdx: number) => (
...
const renderCellContent = (column: string, cell: any, row: any[]) => {
  ...
  if (lowerCol.includes("name") && cellStr) {
    ...
    if (!id && row.length > 1) {
      ...
      if (!firstCell.includes("@") && firstCell.toLowerCase() !== cellStr.toLowerCase()) {
    ...
    if (row.some((value) => value && String(value).includes("@"))) {
      ...
      return <Link href={`/crm/customers/${id}`} className="text-blue-600 hover:underline">{cellStr}</Link>;
    } else if (row.some((value) => value && ["car", "truck", "van", "bike"].includes(String(value).toLowerCase()))) {
      // Probably a vehicle
      id = id || cellStr.toLowerCase().replace(/\s+/g, "-");
      return <Link href={`/fleet/vehicles/${id}`} className="text-blue-600 hover:underline">{cellStr}</Link>;
    } else if (row.some((value) => value && ["electronics", "clothing", "food", "tools"].includes(String(value).toLowerCase()))) {
      // Probably a product
      id = id || cellStr.toLowerCase().replace(/\s+/g, "-");
      return <Link href={`/inventory/products/${id}`} className="text-blue-600 hover:underline">{cellStr}</Link>;
    } else if (row.some((value) => value && ["engineering", "marketing", "sales", "hr"].includes(String(value).toLowerCase()))) {
      // Probably an employee
      id = id || cellStr.toLowerCase().replace(/\s+/g, "-");
      return <Link href={`/hr/employees/${id}`} className="text-blue-600 hover:underline">{cellStr}</Link>;
    } else {
      // Default entity type
      return cellStr;
    }
