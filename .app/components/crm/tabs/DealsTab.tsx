...
export default function DealsTab({ companyId, searchQuery }: DealsTabProps) {
    ...
    async (page: number) => {
      ...
      try {
        ...
        if (!response.success || !response.data) {
          setError(
            toFriendlyCrmError(response.error || "Failed to load deals."),
          );
          return;
        }
  ...
  const gridData = useMemo(
    () => rows.map((row) => normalizeRowForGrid("deals", row)),
    [rows],
  );
  ...
  const handleAddRow = async (payload: Record<string, unknown>) => {
    ...
    try {
      const response = await createCrmRowAction({
        companyId,
        table: "deals",
        standardData: crmViewHelpers.serializeStandardData("deals", payload),
        customData: asRecord(payload.customValues),
      });
      if (!response.success) {
        throw new Error(response.error || "Failed to create deal row.");
      }
  ...
  const handleUpdateRow = async (
    ...
    try {
      ...
      if (!response.success) {
        throw new Error(response.error || "Failed to update deal row.");
      }
      await loadTableView(currentPage);
    } catch (mutationError) {
      setError(
  ...
  const handleDeleteRow = async (rowId: string) => {
    ...
    try {
      ...
      if (!response.success) {
        throw new Error(response.error || "Failed to delete deal row.");
      }
      await loadTableView(currentPage);
    } catch (mutationError) {
      setError(
  ...
  const handleAddColumn = async (column: Omit<VirtualColumn, "id">) => {
    ...
    try {
      const response = await createCrmCustomFieldAction({
        companyId,
        entityType: tableToEntity("deals"),
