...
const getCategoryIcon = (category: string) => {
  switch (category) {
    case 'task': return <FileCheck className="w-4 h-4 text-blue-500" />;
    ...
    case 'approval': return <CheckCircle2 className="w-4 h-4 text-indigo-500" />;
    case 'hr': return <Users className="w-4 h-4 text-pink-500" />;
    case 'system': return <Bell className="w-4 h-4 text-slate-500" />;
...
const getEntityLink = (notification: Notification) => {
  ...
  switch (notification.category) {
    ...
    case 'fleet':
      return metadata.vehicle_id ? `/fleet/vehicles/${metadata.vehicle_id}` : '/fleet';
    case 'inventory':
      return metadata.product_id ? `/inventory/${metadata.product_id}` : '/inventory';
    case 'hr':
      return metadata.employee_id ? `/hr/employees/${metadata.employee_id}` : '/hr';
    default:
  ...
  return (
    <div 
      ...
      className={cn(
        ...
        notification.is_read ? "bg-transparent opacity-75" : "bg-primary/5 hover:bg-primary/10"
      )}
    >
      <div className="flex-shrink-0 mt-1">
        <div className={cn(
          "w-10 h-10 rounded-full flex items-center justify-center border shadow-sm",
          notification.is_read ? "bg-muted border-border" : "bg-background border-primary/20"
        )}>
