...
export const supplierService = {
  ...
  > {
    ...
    if (search) {
      query = query.or(
        `name.ilike.%${search}%,code.ilike.%${search}%,contact_person.ilike.%${search}%,email.ilike.%${search}%,phone.ilike.%${search}%`,
      );
    }
  ...
  async create({
    ...
    supabase: SupabaseClient;
    companyId: string;
    payload: Extract<InventoryCreateRowInput, { table: "suppliers" }>;
  ...
  async update({
    ...
    companyId: string;
    rowId: string;
    payload: Extract<InventoryUpdateRowInput, { table: "suppliers" }>;
  }): Promise<ServiceResult<Record<string, unknown>>> {
