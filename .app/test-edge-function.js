...
async function testReadModuleData(module, filters = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `❌ Error calling readModuleData: ${data.error || "Unknown error"}`,
      );
      return false;
    }
    ...
    console.log(
      `✅ Success: Found ${data.data.length} records in ${module} module`,
    );
...
async function main() {
  ...
  const inventorySuccess = await testReadModuleData("inventory", {
    is_active: true,
  });
