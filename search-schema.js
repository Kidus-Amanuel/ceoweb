...
console.log('\nAll module tables:');
const moduleTables = ['customers', 'deals', 'activities', 'vehicles', 'drivers', 'maintenance', 'products', 'warehouses', 'inventory_movements', 'employees', 'departments', 'leave_requests', 'invoices', 'expenses', 'payments'];
moduleTables.forEach(table => {
  const hasCustomFields = tablesWithCustomFields.includes(table);
  console.log(`- ${table}: ${hasCustomFields ? '✅ custom_fields exists' : '❌ custom_fields missing'}`);
});
